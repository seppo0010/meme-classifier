...
app = Flask(__name__, static_folder=toclassify_path)
...
def index():
    ...
    return f'<img src="/{os.path.basename(toclassify_path)}/{path}" height=600 /><br><a href="classify?image={q(path)}&template=">None</a><br>' + '<br>'.join(f'<a href="classify?image={q(path)}&template={q(s[0])}">{s[0]}</a>' for s in similarities)

@app.route("/classify")
def classify():
    image = request.args['image']
    template = request.args['template'] or 'garbage'
